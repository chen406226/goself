...
func CopyAuthority(c *gin.Context) {
	...
	if err != nil{
		...
		response.OkWithData(resp.SysAuthorityCopyResponse{Authority:authBack},c)
	}
}
...
func SetDataAuthority(c *gin.Context)  {
	...
	if err != nil{
	    response.FailWithMessage(fmt.Sprintf("设置管理失败，%v",err),c)
	} else {
		response.Ok(c)
	}
}